...
if  trim(lowercase(Screen.Forms[I].caption ))='redstone commander' then 
 begin
   ff:=  Screen.Forms[I];
  // xfgform2:=ff as tform;
   for ii := ff.ComponentCount - 1 downto 0 do
    begin
    temp:=nil;
     temp := ff.Components[ii]; 
     
     if (temp is Tfileiconview) then if form2filelist=nil then
     form2filelist:=temp as Tfileiconview ;
     
     if (temp is TListView) then if form2filelistview=nil then
     form2filelistview:=temp as TListView;
